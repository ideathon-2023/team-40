    ...
    if (days_left === 1) {
        return (
            <div className="deadline">
                ...
                <DatePicker showIcon placeholderText="Select a date" selected={selectedDate} onChange={handleChange} dateFormat='dd/MM/yyyy' minDate={new Date().setDate(new Date().getDate() + 1)} maxDate={new Date().setDate(new Date().getDate() + 30)} />
            </div>
        )
    } else {
        return (
